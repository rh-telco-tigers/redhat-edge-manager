...
- **Red Hat Edge Manager** — not automated here; when you are ready, follow [docs/04-rhem-1-on-rhel.md](docs/04-rhem-1-on-rhel.md) and the [product install guide](https://docs.redhat.com/en/documentation/red_hat_edge_manager/1.0/html/installing_red_hat_edge_manager_on_red_hat_enterprise_linux/).  
...
3. [docs/04-rhem-1-on-rhel.md](docs/04-rhem-1-on-rhel.md) — **only when installing RHEM yourself**  
...
Terraform defaults target a **general RHEL 9** guest. For **RHEM** on that same host, Red Hat recommends **4 vCPU / 16 GB RAM** — raise `vm_cores`, `vm_memory_mb`, and `vm_disk_gb` in `terraform.tfvars` before apply (see [04-rhem-1-on-rhel.md](docs/04-rhem-1-on-rhel.md)).
